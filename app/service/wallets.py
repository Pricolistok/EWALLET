...
def get_balance(db: Session, current_user: User,  wallet_name: str | None = None):
    if wallet_name is None:
        return f'total ablance: {sum([i.balance for i in get_total_balance(db, current_user.id)])}'
    if not check_wallet_exist(db, current_user.id, wallet_name=wallet_name):
        raise HTTPException(
            status_code=404,
...
def create_wallet(db: Session, current_user: User, wallet: OperationCreateWallet):
    if check_wallet_exist(db, current_user.id, wallet.wallet_name):
        raise HTTPException(status_code=400, detail=f'Wallet {wallet.wallet_name} already exists')
    creator_wallet = init_wallet(db, current_user.id, wallet.wallet_name, wallet.init_balance)
    db.commit()
